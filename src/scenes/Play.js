class Play extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.spritesheet('explosion2', './assets/explosion copy.png', {frameWidth: 32, frameHeight: 16, startFrame: 0, endFrame: 9});
    ...
    create() {
        ...
        this.FastShip = new FastSpaceship(this, game.config.width, borderUISize*3 + borderPadding*5, 'fastspaceship', 0, 50).setOrigin(0,0);
        // define keys
        ...
        let gameTime = game.settings.gameTimer;
        this.realTime = gameTime/1000;
        scoreConfig.fixedWidth = 0;

        this.timeLeft = this.add.text(borderUISize + borderPadding*50, borderUISize + borderPadding*2.1, this.realTime, scoreConfig);
        this.second = 100;
    ...
    update() {
        ...
        if (this.second == 100 && this.gameOver != true) {
            if (this.realTime > 0) {
                this.realTime -= 1;
            }
            this.timeLeft.text = this.realTime;
            this.second = 0;
        }
        this.second += 1;
        ...
        if (this.checkCollision(this.p1Rocket, this.FastShip)) {
            this.p1Rocket.reset();
            this.shipExplode2(this.FastShip);
        }
    }
